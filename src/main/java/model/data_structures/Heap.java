...
public class Heap <T extends  Comparable<T>> implements IHeap<T>{
    ...
    public void sink(int index, int limit) {
        ...
        while (index * 2 <= limit){
            ...
            if (smallerChildIndex < limit &&
                    arr[smallerChildIndex].compareTo(arr[smallerChildIndex + 1]) < 0){
                smallerChildIndex++;
            }
            if (arr[index].compareTo(arr[smallerChildIndex]) > 0){
                break;
            }
            exchange(index, smallerChildIndex);
    ...
    public void sink(int index, int limit, Comparator c) {
        ...
        while (index * 2 <= limit){
            int smallerChildIndex = index * 2;
            if (smallerChildIndex < limit &&
                    c.compare(arr[smallerChildIndex], arr[smallerChildIndex + 1]) < 0){
                smallerChildIndex++;
            }
            if (c.compare(arr[index], arr[smallerChildIndex]) > 0){
                break;
            }
            exchange(index, smallerChildIndex);
            index = smallerChildIndex;
